...
CREATE SCHEMA IF NOT EXISTS `libraryitego` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `libraryitego` ;
...
CREATE  TABLE IF NOT EXISTS `libraryitego`.`emprestimo` (
  ...
  `avalicao_idavalicao` INT NOT NULL ,
  `multa_idmulta` INT NOT NULL ,
  `patrimonio_idpatrimonio` INT NOT NULL ,
  `usuario_idusuario` INT NOT NULL ,
  PRIMARY KEY (`idemprestimo`) ,
  INDEX `fk_emprestimo_avalicao1_idx` (`avalicao_idavalicao` ASC) ,
  ...
  CONSTRAINT `fk_emprestimo_avalicao1`
    FOREIGN KEY (`avalicao_idavalicao` )
